...
class ADMBike_Woo_Locations_Shipping {
	...
	public function register_shipping_method( $methods ) {
		$methods['orpot_woo_locations'] = 'ADMBike_Woo_Locations_Shipping_Method';
		return $methods;
	}
	...
	public function load_shipping_methods( $package = null ) {
		...
		foreach ( $zones as $zone ) {
			...
			foreach ( $zone_methods as $method ) {
				...
				if ( isset( $method->id ) && 'orpot_woo_locations' === $method->id ) {
					continue;
				}
			}
		}
	...
	protected function get_session_location() {
		...
		if ( isset( WC()->session ) && WC()->session->has_session() ) {
			$session_location = (array) WC()->session->get( 'orpot_woo_locations_checkout_location', array() );
	...
	public function inject_direct_rate( $rates, $package ) {
		...
		foreach ( $rates as $rate_id => $rate ) {
			if ( 0 === strpos( (string) $rate_id, 'orpot_woo_locations' ) ) {
				return $rates;
			}
		}
	...
	protected function build_direct_rate( $package ) {
		...
		$rate->add_meta_data( '_orpot_woo_locations_rule_id', (int) ( $applied_rule['id'] ?? 0 ) );
		$rate->add_meta_data( '_orpot_woo_locations_rule_type', (string) ( $applied_rule['rule_type'] ?? '' ) );
		$rate->add_meta_data( '_orpot_woo_locations_match_type', (string) ( $applied_rule['match_type'] ?? '' ) );
		$rate->add_meta_data( '_orpot_woo_locations_rule_priority', isset( $applied_rule['priority'] ) ? (int) $applied_rule['priority'] : 100 );
		$rate->add_meta_data( '_orpot_woo_locations_rule_specificity', $this->get_match_specificity( (string) ( $applied_rule['match_type'] ?? '' ) ) );
		$rate->add_meta_data( '_orpot_woo_locations_postcode', $postcode );
		$rate->add_meta_data( '_orpot_woo_locations_state_id', $state_id );
		$rate->add_meta_data( '_orpot_woo_locations_municipality_id', $municipality_id );
		$rate->add_meta_data( '_orpot_woo_locations_display_title', $label );
		$rate->add_meta_data( '_orpot_woo_locations_customer_message', $customer_message );
	...
	public function maybe_set_rate_description( $rate, $args, $method ) {
		if ( ! $rate instanceof WC_Shipping_Rate || ! is_object( $method ) || ! isset( $method->id ) || 'orpot_woo_locations' !== $method->id ) {
			return $rate;
		}
	...
	public function prefer_plugin_rates( $rates, $package ) {
		...
		uasort(
			$plugin_rates,
			function ( $left, $right ) {
				$left_specificity  = (int) ( is_object( $left ) && method_exists( $left, 'get_meta' ) ? $left->get_meta( '_orpot_woo_locations_rule_specificity', 99 ) : 99 );
				$right_specificity = (int) ( is_object( $right ) && method_exists( $right, 'get_meta' ) ? $right->get_meta( '_orpot_woo_locations_rule_specificity', 99 ) : 99 );

				if ( $left_specificity !== $right_specificity ) {
					return $left_specificity <=> $right_specificity;
				}

				$left_priority  = (int) ( is_object( $left ) && method_exists( $left, 'get_meta' ) ? $left->get_meta( '_orpot_woo_locations_rule_priority', 100 ) : 100 );
				$right_priority = (int) ( is_object( $right ) && method_exists( $right, 'get_meta' ) ? $right->get_meta( '_orpot_woo_locations_rule_priority', 100 ) : 100 );
