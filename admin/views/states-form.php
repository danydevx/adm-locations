...
<form method="post" action="" style="max-width:600px;">
	...
	<table class="form-table">
		...
		<tr>
			<th scope="row">
				<label for="postcode_coverage_mode"><?php esc_html_e( 'Coverage Mode', 'admbike-woo-locations' ); ?></label>
			</th>
			<td>
				<select id="postcode_coverage_mode" name="postcode_coverage_mode">
					<option value="" <?php selected( '', $selected_coverage_mode ); ?>><?php esc_html_e( 'No limit / WooCommerce state only', 'admbike-woo-locations' ); ?></option>
				...
				<p class="description"><?php esc_html_e( 'Leave empty to sync only the WooCommerce state. Range and List let you restrict available CPs.', 'admbike-woo-locations' ); ?></p>
			</td>
		</tr>
		<tr id="postcode_coverage_range_row" style="<?php echo 'range' !== $selected_coverage_mode ? 'display:none;' : ''; ?>">
			...
			<td>
				...
				<p class="description"><?php esc_html_e( 'Enter the first and last CP in the state range.', 'admbike-woo-locations' ); ?></p>
			</td>
		</tr>
...
(function() {
	...
	function toggleCoverageMode(mode) {
		var hasCoverage = mode === 'range' || mode === 'list';
		if (rangeRow) {
		...
		if (rangeRow && listRow && !hasCoverage) {
